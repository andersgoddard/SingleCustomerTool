...
public class ContactInfoParserImplTest {
    private ContactInfoParser parser;

    @BeforeEach
    public void setUp(){
        parser = new ContactInfoParserImpl();
    }
    @Test
    public void testParsePhoneNumber(){
        List<ContactInfoItem> items = parser.parse("07881266969");
        assertEquals(PhoneNumber.class, items.get(0).getClass());
    }

    @Test
    public void testParseEmailAddress(){
        List<ContactInfoItem> items = parser.parse("andersgoddard@gmail.com");
    ...
    public void testParseMultiplePhoneNumbers(){
        ...
        List<ContactInfoItem> items = parser.parse(multiContactInfo);
